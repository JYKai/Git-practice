...
- ```git log```를 통해 확인해보면 커밋이 취소된 것을 확인할 수 있다.

| 명령       | 설명         |
| :-------- | ----------- |
| ```--soft HEAD^``` | 최신 커밋을 하기 전 상태로 작업 트리를 되돌린다. |
| ```--mixed HEAD^``` | 최근 커밋과 스테이징을 하기 전 상태로 작업트리를 되돌린다. 옵션 없이 ```git reset``` 명령어를 사용할 경우 이 옵션을 기본으로 작동한다. |
...
- ```git log```를 통해 원하는 시점의 커밋 해시를 확인한다.
- 예를 들어, ```reset A```를 입력한다면 이 명령은 A 커밋을 리셋하는 것이 아니라 최근 커밋을 A로 리셋한다. 즉, A 커밋을 삭제하는 것이 아니라 A 커밋 이후에 만들었던 커밋을 삭제하고, A 커밋으로 이동하겠다는 의미이다.

</br>

### 커밋 삭제하지 않고 되돌리기 -- ```git revert```
커밋을 되돌릴 때 수정했던 것을 삭제해도 괜찮다면 ```git reset``` 명령을 사용하면 되지만, 나중에 사용할 것을 대비해서 커밋을 되돌리더라도 취소한 커밋을 남겨두어야 할 때 사용한다.
- ```reset```의 경우에는 취소할 커밋의 해시가 아니라 되돌아갈 커밋 해시를 지정하고, ```revert```의 경우에는 취소할 커밋 해시를 지정한다는 점에 주의하자.
- 로그에 ```revert```한 새로운 커밋이 생긴다. 버전을 지우는 대신 변경했던 이력을 취소한 새 커밋을 만든다.